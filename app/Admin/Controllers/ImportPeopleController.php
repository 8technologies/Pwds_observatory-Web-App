...
class ImportPeopleController extends AdminController
{
    public function import_people_process(Request $request)
    {
        ...
        foreach ($data[0] as $row) {
            ...
            $person->sex = $row[2];
            $person->profiler = $row[9];
            $person->religion = $row[10];
            $person->is_formal_education = $row[11];
            $person->informal_education = $row[12];
            $person->education_level = $row[13];
